...
private:
    ...
    T update(int index, int newValue, int node, int nodeLeft, int nodeRight)
    {
        // 범위를 벗어나면 원래 값 반환
        ...
        // 리프노드이면 값 변경
        if (nodeLeft == nodeRight)
            return range[node] = newValue % MOD;
...
public:
    ...
    // array[index] = newValue로 변경
    T update(int index, int newValue)
    {
